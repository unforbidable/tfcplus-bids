...
public class BlockSaddleQuern extends BlockContainer {
    ...
    public BlockSaddleQuern(Block materialBlock) {
        ...
        if (!world.isRemote) {
            if (heldItemStack == null) {
                ...
                if (saddleQuern.isValidWorkStone(heldItemStack)) {
                    WorkStoneType workStoneType = WorkStoneType.getWorkStoneType(heldItemStack);
                    ...
                    SaddleQuernPlayerEvent event = new SaddleQuernPlayerEvent(player, saddleQuern, SaddleQuernPlayerEvent.Action.PLACE_LEVER, WorkStoneType.SADDLE_QUERN_PRESSING);
                    MinecraftForge.EVENT_BUS.post(event);
    ...
    public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 startVec, Vec3 endVec) {
        ...
        if (mop != null) {
            ...
            if (mop.sideHit == 1) {
                ...
                if (isItemHit) {
                    saddleQuern.setSelection(TileEntitySaddleQuern.Selection.INPUT_STACK);
                } else {
                    saddleQuern.setSelection(TileEntitySaddleQuern.Selection.WORK_STONE);
                }
            } else {
                saddleQuern.setSelection(TileEntitySaddleQuern.Selection.NONE);
            }
        }
