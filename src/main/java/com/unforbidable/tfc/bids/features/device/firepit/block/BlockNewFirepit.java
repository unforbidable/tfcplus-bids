package com.unforbidable.tfc.bids.features.device.firepit.block;
    ...
    private boolean handleInteraction(World world, int x, int y, int z, EntityPlayer entityplayer, int side) {
        ...
        if (equippedItem != null) {
            ...
            if (item instanceof ItemCustomShovel && te.ashNumber > 0 && te.fireTemp <= 1F) {
                TFC_Core.giveItemToPlayer(new ItemStack(TFCItems.powder, te.ashNumber, PowderMeta.ASH), entityplayer);
                te.ashNumber = 0;
