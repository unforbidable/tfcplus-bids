...
public class CrucibleLiquidStorage {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < liquidTags.tagCount(); i++) {
            ...
            addLiquid(MetalHelper.getMetalFromItem(Item.getItemById(id)), volume);
        }
    ...
    public boolean isAllLiquid(float temp) {
        for (CrucibleLiquidItem it : items) {
            if (!MetalHelper.isMeltedAtTemp(it.getMetal(), temp)) {
                return false;
            }
        }
        return true;
    ...
    private void update() {
        ...
        for (CrucibleLiquidItem it : items) {
            ...
            heatCapacity += MetalHelper.getHeatCapacity(new ItemStack(it.getMetal().ingot)) * it.getVolume();
        }
