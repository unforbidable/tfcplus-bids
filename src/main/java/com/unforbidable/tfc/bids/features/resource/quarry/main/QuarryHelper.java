...
public class QuarryHelper {
    ...
    public static int getSideRequiringWedgesCount(World world, int x, int y, int z, Quarriable quarriable, int side) {
    ...
    public static boolean canQuarryBlockAt(World world, int x, int y, int z, Block block, int side) {
        Quarriable quarriable = findQuarriable(block);
    ...
    public static Quarriable findQuarriable(Block block) {
        return QuarryRegistry.quarriable.stream()
            .filter(q -> q.getRawBlock() == block)
            .findAny().orElse(null);
    }
