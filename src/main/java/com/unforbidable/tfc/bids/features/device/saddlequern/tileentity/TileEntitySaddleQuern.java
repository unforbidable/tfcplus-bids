package com.unforbidable.tfc.bids.features.device.saddlequern.tileentity;
...
public class TileEntitySaddleQuern extends TileEntity implements IInventory {
    ...
    public WorkStoneType getWorkStoneType() {
        if (!isCachedWorkStoneTypeValid) {
            if (hasWorkStone()) {
                cachedWorkStoneType = WorkStoneType.getWorkStoneType(storage[SLOT_WORK_STONE]);
            } else {
                cachedWorkStoneType = WorkStoneType.NONE;
            }
    ...
    public boolean isValidInput(ItemStack inputStack) {
        if (getWorkStoneType() == WorkStoneType.SADDLE_QUERN_CRUSHING) {
            return SaddleQuernRegistry.recipes.findMatchingRecipe(inputStack) != null;
        } else if (getWorkStoneType() == WorkStoneType.SADDLE_QUERN_PRESSING) {
            return StonePressRegistry.recipes.findMatchingRecipe(inputStack) != null;
        } else {
            return false;
        }
    }
    ...
    private boolean isPressActive() {
        if (getWorkStoneType() == WorkStoneType.SADDLE_QUERN_PRESSING) {
    ...
    private boolean processPressingInput() {
        if (storage[SLOT_INPUT_STACK] != null) {
            ...
            StonePressRecipe recipe = StonePressRegistry.recipes.findMatchingRecipe(storage[SLOT_INPUT_STACK]);
            if (recipe != null) {
    ...
    private void processInput() {
        ...
        if (storage[SLOT_INPUT_STACK] != null && storage[SLOT_OUTPUT_STACK] == null) {
            ItemStack input = storage[SLOT_INPUT_STACK];
            SaddleQuernRecipe recipe = SaddleQuernRegistry.recipes.findMatchingRecipe(input);
            if (recipe != null) {
