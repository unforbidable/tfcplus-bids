...
public abstract class TileEntityCrucible extends TileEntity implements IInventory, ISlotTracker, LiquidMetalContainer {
    ...
    public int getGlassMakingRemainingHours() {
        return (int) Math.ceil(getGlassMakingProgress() * 8);
    }
    ...
    public boolean isItemStackValidInput(ItemStack is) {
        return is.getItem() instanceof ISmeltable
                ...
                && MetalHelper.isMeltedAtTemp(is, getMaxTemp())
                && !MetalHelper.isOreIron(is)
                || MetalHelper.isGlassIngredient(is)
                || MetalHelper.isGlass(is);
    }
    ...
    public boolean isItemStackValidLiquidOutput(ItemStack is) {
        return MetalHelper.isValidMold(is, null);
    }
    ...
    public int acceptLiquidMetal(Metal metal, int volume, float temp) {
        if (canAcceptLiquidMetal(metal, 1, temp)) {
            ...
            float addedMetalHeatCapacity = MetalHelper.getHeatCapacity(new ItemStack(metal.ingot)) * volume;
    ...
    private void updateOutput() {
        ...
        for (int i = 0; i < getInputSlotCount(); i++) {
            ...
            if (is != null) {
                predictedLiquidStorage.addLiquid(MetalHelper.getMetalFromSmeltable(is),
                        MetalHelper.getMetalReturnAmount(is) * is.stackSize);
    ...
    private boolean canAcceptLiquid() {
        ...
        return hasLiquidInputSlot()
                ...
                && MetalHelper.isMeltedAtTemp(is, getMaxTemp())
                && MetalHelper.isMeltedAtTemp(is, TFC_ItemHeat.getTemp(is))
                && isValidInputMetal(MetalHelper.getMetalFromItem(is.getItem()));
    }
    ...
    private boolean canEjectLiquid() {
        return inputMonitor.getVolume() == 0 && liquidStorage.getVolume() > 0
                ...
                && MetalHelper.isValidMold(storage[getLiquidOutputSlotIndex()], liquidStorage.getOutputMetal())
                && !canAcceptLiquid();
    }
    ...
    private void doWork(boolean updateSolidTemp, boolean updateLiquidTemp, boolean acceptLiquid, boolean ejectLiquid,
            ...
            if (updateLiquidTemp) {
                if (liquidStorage.getVolume() > 0
                    ...
                    if (heatSourceTemp > liquidTemp) {
                        deltaFromHeatSource *= CrucibleConfig.liquidHeatingMultiplier;
                    } else {
                        deltaFromHeatSource *= CrucibleConfig.coolingMultiplier;
                    }
            ...
            if (updateSolidTemp) {
                if (inputMonitor.getVolume() > 0
                    ...
                    if (heatSourceTemp > solidTemp) {
                        deltaFromHeatSource *= CrucibleConfig.solidHeatingMultiplier;
                        deltaFromLiquid *= CrucibleConfig.solidHeatingFromLiquidBonusMultiplier;
                    } else {
                        deltaFromHeatSource *= CrucibleConfig.coolingMultiplier;
                        deltaFromLiquid *= CrucibleConfig.coolingMultiplier;
                    }
            ...
            if (updateSolidTemp) {
                ...
                if (glassCanBeCreated || liquidStorage.isAllLiquid(liquidTemp)) {
                    for (int i = 0; i < getInputSlotCount(); i++) {
                        ItemStack is = getStackInSlot(i);
                        if (is != null) {
                            if (MetalHelper.isMeltedAtTemp(is, solidTemp)
                                    && !MetalHelper.isOreIron(is)
                                    && !MetalHelper.isGlassIngredient(is)
                                    || glassCanBeCreated && MetalHelper.isGlassIngredient(is)) {
                                float prevLiquidStorageHeatCapacity = liquidStorage.getHeatCapacity();
                                Bids.LOG.debug("Input item stack " + is.getUnlocalizedName() + "[" + is.stackSize
                                        + "] melted");
                                liquidStorage.addLiquid(MetalHelper.getMetalFromSmeltable(is),
                                        MetalHelper.getMetalReturnAmount(is) * is.stackSize);
                                ...
                                float meltedStackHeatCapacity = MetalHelper.getHeatCapacity(is)
                                        * MetalHelper.getMetalReturnAmount(is) * is.stackSize;
                                ...
                                if (inputMonitor.getVolume() == 0) {
                                    ...
                                    if (CrucibleConfig.furnaceOverheatingRuinChance > worldObj.rand.nextFloat()
        ...
        if (updateSolidTemp || updateLiquidTemp) {
            ...
            if (!CrucibleConfig.enableExactTemperatureDisplay)
        ...
        if (acceptLiquid) {
            ...
            if (hasLiquidInputSlot()
                ...
                if (liquidInputStack != null && liquidInputStack.getItem() instanceof ItemMeltedMetal
                        && MetalHelper.isMeltedAtTemp(liquidInputStack, getMaxTemp())
                        && MetalHelper.isMeltedAtTemp(liquidInputStack, TFC_ItemHeat.getTemp(liquidInputStack))) {
                    Metal inputMetal = MetalHelper.getMetalFromItem(liquidInputStack.getItem());
                    if (isValidInputMetal(inputMetal)) {
                        ItemStack newMold = MetalHelper.drainMold(liquidInputStack, 1);

                        int prevUnits = MetalHelper.getMoldUnits(liquidInputStack);
                        int newUnits = MetalHelper.getMoldUnits(newMold);
                        ...
                        // Combine melted stack temp with liquid temp
                        float addedLiquidHeatCapacity = MetalHelper.getHeatCapacity(liquidInputStack);
        ...
        if (ejectLiquid) {
            ...
            if (inputMonitor.getVolume() == 0 && liquidStorage.getVolume() > 0
                ...
                if (liquidOutputStack != null
                        && MetalHelper.isValidMold(liquidOutputStack, liquidStorage.getOutputMetal())) {
                    Bids.LOG.debug("Ejecting 1 unit of: " + liquidStorage.getOutputMetal().name);

                    ItemStack newMold = MetalHelper.fillMold(liquidOutputStack, liquidStorage.getOutputMetal(), 1);
                    ...
                    int prevUnits = MetalHelper.getMoldUnits(liquidOutputStack);
                    int newUnits = MetalHelper.getMoldUnits(newMold);
                    ...
                    if (liquidStorage.getVolume() == 0
                            || !MetalHelper.isValidMold(newMold, liquidStorage.getOutputMetal())) {
