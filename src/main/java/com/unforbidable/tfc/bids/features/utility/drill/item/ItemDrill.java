package com.unforbidable.tfc.bids.features.utility.drill.item;
...
public class ItemDrill extends ItemCommonTool {
    ...
    public int getMaxDamage(ItemStack stack) {
        return (int) (getMaxDamage()+(getMaxDamage() * TfcUtil.getItemStackDurabilityBuff(stack)));
    }
    ...
    public boolean canPlayerDrillBlock(Block block, int metadata, EntityPlayer player) {
        ...
        Quarriable quarriable = QuarryHelper.findQuarriable(block);
        if (quarriable == null) {
    ...
    protected int getBaseDrillDuration() {
        return Math.round(QuarryConfig.baseDrillDuration * durationMultiplier);
    }
    ...
    protected int getDrillDuration(ItemStack stack, EntityPlayer player) {
        ...
        float bonus = 1 - Math.min(1, TfcUtil.getItemStackDurabilityBuff(stack)) * 0.25f;
    ...
    protected void onBlockDrilled(World world, int x, int y, int z, int side, ItemStack stack, EntityPlayer player) {
        ...
        } else if (QuarryHelper.findQuarriable(block) != null) {
            int metadata = world.getBlockMetadata(x, y, z);
    ...
    protected ItemStack onDrillDestroyed(ItemStack stack, EntityPlayer player) {
        ...
        boolean bowstringBroke = QuarryConfig.bowStringBreakChance > player.worldObj.rand.nextDouble();

        // Require binding if previous drill had binding
        boolean requireBinding = TfcUtil.getItemStackDurabilityBuff(stack) > 0;

        if (QuarryConfig.enableDrillAutoRepair) {
