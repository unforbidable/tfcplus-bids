...
public class RenderCookingPotLid implements ISimpleBlockRenderingHandler {

    @Override
    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
        for (int i = 0; i < 2; i++) {
            renderPartInv(renderer, block, metadata, CookingPotBounds.LID_ONLY_BOUNDS[i]);
        }
    }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        for (int i = 0; i < CookingPotBounds.LID_ONLY_BOUNDS.length; i++) {
            renderPart(renderer, x, y, z, block, CookingPotBounds.LID_ONLY_BOUNDS[i]);
        }
