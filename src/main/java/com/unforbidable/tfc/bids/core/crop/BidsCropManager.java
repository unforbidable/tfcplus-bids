...
public class BidsCropManager {
    ...
    private static final List<BidsCropIndex> crops = new ArrayList<>();
    private static final Map<Integer, CropRegion> regionalCrops = new HashMap<>();

    private static final Map<Integer, BidsCropIndex> cropsById = new HashMap<>();
    private static final Map<String, BidsCropIndex> cropsByName = new HashMap<>();
    ...
    static void registerCrop(BidsCropIndex cropIndex) {
        ...
        if (cropIndex.regions != null && cropIndex.commonness > 0) {
            ...
            for (EnumRegion region : EnumRegion.values()) {
                if (cropRegions.contains(region)) {
                    if (cropIndex.coastal == CropCoastAffinity.COAST_ONLY) {
                        regionalCrops.get(region.ordinal()).getCoastalCrops().addAll(weighted);
                    } else if (cropIndex.coastal == CropCoastAffinity.INLAND_ONLY) {
